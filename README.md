...
.venv/bin/pytest -q                    # 75 tests; Postgres cases skip if the DB is down
...
`DATABASE_URL` / `TEST_DATABASE_URL` override the default connection strings.
...
| Wiring / UI | `app/container.py`, `app/cli.py` | Dependency wiring, argparse CLI |
...
- **Rates**: Hatchback uses the spec example exactly (min ₹50, 10/8/5). Sedan: min ₹60, 12/10/7.
...
- A user can have at most one ongoing ride. The search radius defaults to 5 km.
- Cancellation is free within 2 minutes of booking, ₹25 after that.
- No auth, payments, driver acceptance step, or driver on/offline toggle.

## Key design decisions & trade-offs

- **Strategies at each variation point.** Fare per car type, surge, matching and cancellation are all interfaces injected into the services. Adding an SUV means one `CarType` value plus one `FareStrategy` entry in `DEFAULT_FARE_STRATEGIES`, with no booking code touched. The matching strategy can be set globally (`build_container(matching=...)`) or per booking (`--strategy`).
- **Upgrades are a table** (`UPGRADE_PATH`), not an `if` inside booking logic.
